...
# Linux内存管理——页框管理
Linux采用4KB页框尺寸作为标准内存分配单元。

分页系统造成的缺页异常很好解释，要么是页存在但process不被允许访问，要么是页不存在。如果不存在，内存分配器需要找一个free 4KB页框给process。

尽管4K和4M都是磁盘块尺寸的倍数，但内存和磁盘间往往传递小块更为的高效。

![](/images/ulk/20171020_1.jpg)

## 三个结构
### page
内核记录了每个页框的状态：属于哪个进程；属于内核代码或数据结构；空闲等等。页框的状态保存在页描述符，其类型为page。所有的页描述符都存在mem_map数组。

### NUMA - node
不是所有的内存都是均质的，不同架构寻址内存的不同空间速度是不同的（Alpha,mips,etc.）。所以内存的使用布局就很有讲究，2.6支持NUMA，物理内存被瓜分成多个nodes。不同的node中的page访问速度是相同的。

每个node都可以分成多个zones，每个node有个类型为pg_data_t的描述符，所有node描述符都存储在单一链，第一个元素由pgdat_list变量所指向。
...
2. 现代32位计算机有很多RAM，CPU不能直接访问所有的物理内存，因为线性地址空间太小了。

为了应付这两个限制，Linux分割物理内存node成3个zones。在80x86 UMA架构中如下：
- ZONE_DMA
	- 包含低16MB的内存页框
- ZONE_NORMAL
	- 包含16MB到896MB的内存页框
...
ZONE_DMA给旧ISA总线设备用。ZONE_DMA和ZONE_NORMAL包含内存的常规页框，通过线性映射到4GB的线性地址空间，它们可以被内核直接使用。ZONE_HIGHMEM包含不能够直接访问的内存页框。ZONE_HIGHMEN zone在64bit机器上总是空的。
...
#endif
```
mem_map在node_alloc_mem_map中被赋值，指向了contig_page_data.node_mem_map。再找node_mem_map成员：
```cpp
/**
...
page中有到node和zone的链接，为了省空间，并没有设定单独的指针指向，而是编码成索引放在flags字段的高位。源码中可以看到flags的标志数目是有限的，flags字段的高位可以用于编码特定node和zone号（对32位非NUMA，zone索引2位，node索引1位。（因为zone有3个而node只有1个））。
...
//只关心non-NUMA
```
内核调用内存分配函数时，必须指明请求page所在的zone。内核一般指明它想用的zone。内核使用zonelist数组指明首选的zone：
```cpp
/*
...
## 保留的页框
一方面内存有时会不够用，一些操作会被阻塞到有其他内存free。另一方面内核的某些操作是非常不想阻塞的，所以折中的办法就是留一个保留区。保留内存的数量由min_free_kbytes决定，单位为KB。
```cpp
/**
 * 内核保留内存池大小。
...
## Zoned Page Frame分配器
zoned page frame allocator是一个内核子系统，处理一组连续的页框分配请求。

**概要：**
allocator接收分配和释放请求。分配时，组件搜索包含连续page frames的zone来满足请求。在每个zone内，page frames由组件"buddy system"控制。为了得到更高的性能，少量的page frames放在缓存中以满足多次对单个页面的分配请求。
...
```
alloc_pages_node只是做心智健全检查，实际工作交给了__alloc_pages，注意参数中zonelist是通过NODE_DATA(nid)取出`#define NODE_DATA(nid)		(node_data[nid])`，全局的`extern struct pglist_data *node_data[];`。__alloc_pages涉及了伙伴系统的策略，后面再看。

Appendix gfp_mask flags:
```cpp
/*
 * GFP bitmasks..
 */
...
```
核心在于free_pages_bulk，这涉及到伙伴系统的策略，过后再研究。
对于order不为0的，核心依然是free_page_bulk:
```cpp
...
## ZONE_HIGHMEM内核映射
high_memory变量存放HIGHMEM起始地址，设置为896M。896MB边界以上的page frame不会映射在内核线性地址空间的第4GB空间，因此内核无法直接访问。因此，__get_free_pages等函数对此情形并不适用（32位的系统中，因为__get_free_pages()无法返回一个不存在的线性地址转而返回NULL，会造成page frame的丢失，64位线性地址）。

为了32位系统内核可以使用HIGHMEM，做了一种映射的机制。约定如下：
1. 只用alloc_pages,alloc_page分配HIGHMEM的内存页框，虽然线性地址不存在，但是页描述符page的线性地址存在，这个东西在一开始就分配到NON-HIGHMEM区了。
2. 内核线性地址空间的最后128MB划分出来用于映射HIGHMEM的页框。线性地址对页框是一对多的，映射是暂时性的。

page frame到线性地址的映射方式有3种机制：永久内核映射、临时内核映射和非连续内存分配。目前只关心前两种。

### 永久内核映射
高端页框到内核地址空间的长期映射。使用master kernel page tables中的专门页表，组织结构关系如下：
```cpp
/**
 * 用于建立永久内核映射的页表。
...
### 临时内核映射
临时内核映射可以用在中断处理程序和可延迟函数内部，它们从不阻塞当前进程。
...
## CPU页框高速缓存
内核经常请求或释放单个页框（对应order=0的情况），为了提升性能，内存管理区定义了一个每CPU页框高速缓存用于包含一些预先分配的页框。
Linux为每个内存管理区和每CPU提供了两个高速缓存：热高速和冷高速。两种类型针对不同的请求而设计。

zone的pageset成员保存了两个高速缓存：
```cpp
/**
 * 管理区每CPU页框高速缓存描述符
 */
...
### alloc
buffered_rmqueue()中的buffered实际上就是指高速缓存：
```cpp
/**
...
可以看到利用pcp的几个成员，在`if(order==0)`中完成了高速缓存的处理。如果高速缓存实在搞不定，就一并落入到`__rmqueue()`中处理，`buffered_rmqueue`还包含了对返回页框块的第一个页框的描述符page进行初始化的代码，还可以看到`__GFP_ZERO`的初始化填充0就是这里处理的。
...
```
可以看到free的操作就是和alloc切好相反，通过`list_add`和`pcp->count`搞定。对应的，和alloc时通过pcp->low字段检测到如果页框不够就用`__rm_queue`分配一样，释放时，也会利用`pcp->high`来检测是否当前缓存页框太多了，如果多久释放给buddy system。`free_pages_bulk`就是zone的free函数，下节展开。

## zone alloc/free
### free
那就先看free吧，这个比alloc简单多了。还是回过头来看`__free_pages()`：
```cpp
/**
 * 首先检查page指向的页描述符。
 * 如果该页框未被保留，就把描述符的count字段减1
...
`put_page_testzero`完成对count的减一操作。`order==0`的情形已经了解，看`else`:
```cpp
void __free_pages_ok(struct page *page, unsigned int order)
{
	LIST_HEAD(list);
...
```
大于0时就将页框加到本地链表list，然后再通过此前读过的`free_pages_bulk()`释放它们到zone的buddy system。

### alloc
alloc很复杂，毕竟free时页框是单一具体的，但alloc时却有着五花八门的方式：
```cpp
/**
...
```
`buffered_rmqueue`已经了然，这个判断函数`zone_watermark_ok`是要作甚？
```cpp
/**
 * zone_watermark_ok辅助函数接收几个参数，它们决定内存管理区中空闲页框个数的阀值min。
...
```
可以看到实际上是根据传入的order，阈值以及一些标志等来判断当前所在的zones[i]是否可以分配。zones[i]在Linux中对应HIGH,NORMAL和DMA三块。

要点：
1. 第一次扫描用的是pages_low，这是回收页框使用的下界，zone分配器的阈值，如果第一次扫描不到可用的zone就进行二次扫描。
2. 第二次扫描时，用到的是pages_min，也就是保留页的数目。
3. 如果两次都扫不到，先判断本次需要分配的page的flags中是否有PF_MEMALLOC以及thread flag中TIF_MEMDIE置位以及非中断处理程序或可延迟函数。满足则不通过`watermask_ok`判断，直接`buffered_rmqueue()`分配。
4. 如果实在找不到，flag中又没有`__GFP_WAIT`，那就对不起，返回NULL。否则阻塞，让CPU，等回收页框。
5. 当再次try to free pages时依然失败，则再扫描一次zones（这里的扫描如果只有一个内核控制路径步入，是没有意义的，但对于多内核控制路径是有意义的，因为二者只要有一个也只需要一个通过步入到后方的out_of_memory即可干掉回复page frames）。

经典的out_of_memory：
```cpp
/*
  * 当空闲内存是否紧缺且PFRA又无法成功回收任何页时，__alloc_pages()调用
  */
...
retry:
    ...
    /*
     * 杀死被选择的进程
     * 发送是我信号(通常是SIGKILL)，该信号发给该进程的一个子进程，或如果做不到，就发送给进程本身
     * 同时也删除与被选进程共享内存描述符的所有克隆进程
     */
...
```
## 小结
到此，关于页框的管理策略，从zoned alloc/free system到((per CPU cache)+buddy system)，一系列的算法已经了然于胸。

ULK的这张图很透彻：
![](/images/ulk/20171020_3.jpg)